...
async function getChannelMessages() {
  ...
  // Если переменные окружения не найдены, спрашиваем у пользователя
  if (!envApiId || !envApiHash || !envSession) {
    console.log("Environment variables not found. Please enter the values:");
    apiId = parseInt(await question.prompt("Enter your API ID: "));
    apiHash = await question.prompt("Enter your API HASH: ");
    sessionString = await question.prompt("Enter your SESSION string: ");
  } else {
  ...
  const channelInput = await question.prompt("Enter channel name or ID (e.g., @channel_name or -1001234567890): ");
