...
async def classify_message_with_ollama(message_content: str) -> list:
    """
    Анализирует сообщение с помощью Ollama, используя единый JSON промпт.
    Возвращает список событий (даже если событие одно).
    """
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            print_error(f"  Ошибка при классификации Ollama: {e}")
            return []
                ...
                try:
                    ...
                    for thread_id in thread_ids_to_process:
                        ...
                        for msg in current_messages:
                            # Мы можем классифицировать только сообщения с текстом
                            if msg.text:
                                ollama_results = await classify_message_with_ollama(msg.text)

                                # Обрабатываем каждое найденное событие (может быть несколько дат)
                                for event_data in ollama_results:
                                    if event_data and event_data.get('is_event'):
                                        total_events_classified += 1
                                        
                                        # Логика загрузки изображения (только для событий)
                                        image_url = None
                                        if msg.photo:
                                        ...
                                        if thread_id_param is not None and thread_id_param != 1:
                                            ...
                                            post_link = f"{base_link}/{msg.id}"
                                                                                ...
                                                                                # Совмещаем данные из Ollama с системными полями
